...
enemy::enemy(BITMAP* newEnemySprite,
             BITMAP* newEnemyHurt,
             world* newGameWorld) {
...
void enemy::update() {
  ...
  if (fire_rate < fire_timer) {
    game_world->create_projectile(
        x + 100, y + 30, HELICOPTER,
        find_angle(x + 100, y + 30, game_world->get_character_x() + 30,
                   game_world->get_character_y() + 15),
        5, false, 5, 5);
    fire_timer = 0;
  }
  ...
  for (unsigned int i = 0; i < game_world->get_projectiles()->size(); i++) {
    if (collision(x, x + width, game_world->get_projectiles()->at(i)->get_x(),
                  game_world->get_projectiles()->at(i)->get_x() +
                      game_world->get_projectiles()->at(i)->get_width(),
                  y, y + height, game_world->get_projectiles()->at(i)->get_y(),
